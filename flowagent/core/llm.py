...
"""
    ...
    async def _create_inferred_download_workflow(
        ...
        system = (
            ...
            "- ENCODE — DO NOT hardcode ENCFF file accessions from "
            "memory; they 404 reliably because each download has "
            "different IDs that vary by replicate. ALWAYS resolve "
            "them via the ENCODE JSON API. Pattern:\n"
            "    curl -s 'https://www.encodeproject.org/experiments/"
            "<ENCSR_ID>/?format=json' \\\n"
            "      | python3 -c 'import json,sys; d=json.load(sys.stdin); "
            "print(\"\\n\".join(\"https://www.encodeproject.org\" + f[\"href\"] "
            "for f in d[\"files\"] if f.get(\"file_format\")==\"fastq\" and "
            "f.get(\"output_category\") in (\"raw data\",)))' \\\n"
            "      > raw_data/encode_fastq_urls.txt\n"
            "    wget -nc -P raw_data -i raw_data/encode_fastq_urls.txt\n"
            "  This works for any ENCSR* / ENCFF* / ENCBS* accession "
            "without needing to recall specific file IDs.\n"
            ...
            "- Download steps MUST be idempotent so re-running the "
            "cell after a partial failure doesn't duplicate or "
            "corrupt data:\n"
            "    wget -O <fixed-path>     (NOT bare wget — that "
            "appends ``.1`` / ``.2`` suffixes when the file exists, "
            "and the next ``unzip``/``tar`` step references the "
            "original path which is now stale or corrupt)\n"
            "    unzip -o -d <dir> <zip>  (force overwrite — non-"
            "interactive shells hang on the ``replace? [y]es,[n]o`` "
            "prompt without ``-o``)\n"
            "    tar --overwrite -xf …    (same reason)\n"
            ...
            "path'; exit 1; }\n"
            "- ALIGNER STEPS MUST AUTO-DETECT PAIRED vs SINGLE-END. "
            "Public datasets are not uniformly PE — much of the older "
            "ENCODE corpus (including ENCSR000EUQ) is SE; ENA mirrors "
            "expose SE accessions named without a ``_1`` suffix; "
            "fasterq-dump emits SE data as ``<acc>_1.fastq.gz`` on "
            "some sra-tools versions. NEVER assume PE based on the "
            "presence of ``_1.fastq.gz`` filenames alone. The "
            "canonical bowtie2 / bwa / STAR / kallisto idiom is:\n"
            "    if ls raw_data/${srr}_1.fastq.gz raw_data/${srr}_2.fastq.gz >/dev/null 2>&1; then\n"
            "      <aligner> ... -1 raw_data/${srr}_1.fastq.gz -2 raw_data/${srr}_2.fastq.gz ...\n"
            "    elif [ -f raw_data/${srr}.fastq.gz ]; then\n"
            "      <aligner> ... -U raw_data/${srr}.fastq.gz ...\n"
            "    elif [ -f raw_data/${srr}_1.fastq.gz ]; then\n"
            "      # _1-only — fasterq-dump SE quirk; treat as SE.\n"
            "      <aligner> ... -U raw_data/${srr}_1.fastq.gz ...\n"
            "    else echo 'FAIL: no FASTQ for '${srr}; exit 1; fi\n"
            "  For ENCODE plans: query the ``paired_end`` field of "
            "each file in the API response to know layout upfront. "
            "Files with ``paired_end=='1'`` are R1, ``'2'`` are R2, "
            "missing/null means single-end. Group accordingly when "
            "writing the chip/input FASTQ lists.\n"
            ...
            "``2> path/to/X.log``. The executor captures stderr and "
            "shows it in the workflow log when a step fails; "
            "redirecting it to a file produces empty error messages "
            "and forces every recovery attempt to guess in the dark "
            "(we hit this on bowtie2 silently failing because its "
            "``does not exist or is not a Bowtie 2 index`` error went "
            "to a log file). If you genuinely need the log file too, "
            "use ``2> >(tee -a X.log >&2)`` so stderr also reaches "
            "the executor.\n"
    ...
    async def _generate_analysis_steps_via_llm(
        ...
        system = (
            ...
            "``2> path/to/X.log``. The executor captures stderr; "
            "files swallow the actual error message and leave the "
            "workflow log saying ``Error:`` (empty) when something "
            "fails. Recovery attempts then hallucinate fixes "
            "blindly. If you genuinely need a per-step log, use "
            "``2> >(tee -a X.log >&2)`` so stderr ALSO reaches the "
            "executor.\n"
            "8. DOWNLOAD STEPS MUST BE IDEMPOTENT. Bare ``wget URL`` "
            "appends ``.1``/``.2`` suffixes when the target exists, "
            "and the next ``unzip``/``tar`` step then references the "
            "stale (possibly corrupt) original. Use ``wget -O "
            ...
            "exit 1; }\n"
            "10. ALIGNER STEPS MUST AUTO-DETECT PAIRED vs SINGLE-END. "
            "Public datasets are not uniformly PE — much of the older "
            "ENCODE corpus (including ENCSR000EUQ) is SE; some ENA "
            "mirrors expose SE accessions named without ``_1``; "
            "fasterq-dump emits SE data as ``<acc>_1.fastq.gz`` on "
            "some sra-tools versions. NEVER assume PE based on "
            "filename presence alone. The canonical idiom for "
            "bowtie2 / bwa / STAR / kallisto is:\n"
            "    if [ -f raw_data/${srr}_1.fastq.gz ] && "
            "[ -f raw_data/${srr}_2.fastq.gz ]; then\n"
            "      <aligner> ... -1 raw_data/${srr}_1.fastq.gz "
