...
class NextflowGenerator(PipelineGenerator):
    ...
    def _render_process(
        ...
        #
        # Inside triple-double-quoted Groovy strings, ``$(...)`` and ``$var``
        # are interpreted as Groovy interpolation, which breaks shell
        # substitutions like ``$(grep …)`` or ``$gsm_id``. Escape every
        # ``$`` in the user-supplied command so the shell — not Groovy —
        # evaluates it. The ``${launchDir}`` line is emitted separately
        # and intentionally left unescaped.
        shell_safe = command.replace("\\", "\\\\").replace("$", "\\$")
        lines.append("    script:")
        lines.append('    """')
        lines.append("    cd '${launchDir}'")
        lines.append(f"    {shell_safe}")
        lines.append('    """')
        lines.append("}")
        return "\n".join(lines)
